...
<xsl:template name="generate_footer">
  <xsl:call-template name="generate_header_footer"/>
  <p class="versions"><xsl:value-of select="timestamp"/><xsl:text>, </xsl:text><xsl:value-of select="commit_id"/></p>
  <p class="copyconf">Copyright &#169; 2010-2026 Arm Limited or its affiliates. All rights reserved. This document is Non-Confidential.</p>
</xsl:template>
  ...
  <xsl:choose>
    <xsl:when test="count(access_mechanisms/access_mechanism) = 0 and (count(reg_address) > 0 or count(reg_encoding) > 0)">
      ...
      <xsl:apply-templates select="access_mechanisms/access_permission_text">
        <xsl:with-param name="reg_or_inst" select="$REG_OR_INSTRUCTION"/>
      </xsl:apply-templates>
      <xsl:choose>
  ...
  <xsl:if test="reg_address">
    <xsl:for-each select="reg_address">
      <xsl:call-template name="offset_accessibility">
        <xsl:with-param name="reg_or_inst" select="$reg_or_inst"/>
...
<xsl:template name="offset_accessibility">
  ...
  <xsl:choose>
    <xsl:when test="count($reg_address/reg_access/reg_access_state) = 1 and $reg_address/reg_access/reg_access_state/reg_access_level"> <!-- conditional single access -->
      <p><xsl:value-of select="$reg_address/reg_access/reg_access_state/reg_access_level"/><xsl:text>, accesses to this </xsl:text><xsl:value-of select="$reg_or_inst"/><xsl:text> </xsl:text><xsl:apply-templates select="$reg_address/reg_access/reg_access_state/reg_access_type"/>.</p>
    </xsl:when>
      ...
      <ul>
        <xsl:for-each select="$reg_address/reg_access/reg_access_state">
          <li>
            ...
            <xsl:text>accesses to this </xsl:text><xsl:value-of select="$reg_or_inst"/><xsl:text> </xsl:text><xsl:apply-templates select="reg_access_type"/>.
          </li>
        </xsl:for-each>
      </ul>
    </xsl:when>
    <xsl:otherwise><p>Accesses to this <xsl:value-of select="$reg_or_inst"/><xsl:text> </xsl:text><xsl:apply-templates select="$reg_address/reg_access/reg_access_state/reg_access_type"/>.</p></xsl:otherwise> <!-- unconditional single access -->
  </xsl:choose>
</xsl:template>
...
<xsl:template match="reg_fieldset">
  ...
  <xsl:variable name="FIELDSET_NAME" select="fields_name"/>
  <xsl:variable name="CONDITION">
    <xsl:choose>
  ...
  <xsl:choose>
    <xsl:when test="fields_condition">
      <xsl:if test="$apply_condition != 0">
        <xsl:choose>
          <xsl:when test="$FIELDSET_NAME">
            <!-- ASSUMPTION: condition always starts with "When" or "Otherwise" -->
            <h3><xsl:value-of select="$FIELDSET_NAME"/> variant <xsl:value-of select="concat(translate(substring($CONDITION, 1, 1), 'WO', 'wo'), substring($CONDITION, 2))"/>:</h3>
          </xsl:when>
          <xsl:otherwise>
    ...
    <xsl:otherwise>
      <xsl:if test="$FIELDSET_NAME"><h3><xsl:value-of select="$FIELDSET_NAME"/> variant</h3></xsl:if>
      <xsl:call-template name="reg_diagram">
        <xsl:with-param name="fieldset" select="."/>
      </xsl:call-template>
...
<xsl:template name="reg_diagram">
  <xsl:param name="fieldset"/>
  <!-- Find the matching fieldset id by checking the ids of the first field in each set -->
  <xsl:variable name="FIRST_FIELD_ID"><xsl:value-of select="fieldat/@id"/></xsl:variable>
  <xsl:variable name="FIELDSET_ID"><xsl:value-of select="../fields[field/@id = $FIRST_FIELD_ID]/@id"/></xsl:variable>
  <xsl:choose>
    <xsl:when test="@length &gt; 64">
      <table class="regdiagram">
        <xsl:attribute name="id"><xsl:value-of select="$FIELDSET_ID"/></xsl:attribute>
        <xsl:call-template name="displayArbitraryDiagram">
  ...
  <xsl:for-each select="field[not(./@is_expansion = 'True')]">
    ...
    <xsl:if test="field_description">
      ...
      <xsl:variable name="LSB">
        ...
        <xsl:variable name="CONFLICT" select="
          $FIELD_NAME != 'IMPLEMENTATION DEFINED' and
          parent::*/fields_name and
          //field[string(field_name) = $FIELD_NAME and
            (string(field_msb) != $MSB or string(field_lsb) != $LSB)]" />
          ...
          <xsl:when test="field_name"> <!-- If a field name is defined -->
              <xsl:variable name="HEADER_TEXT">
                <xsl:choose>
                  <xsl:when test="$CONFLICT">
                    <xsl:value-of select="ancestor::fields/fields_name"/>.<xsl:value-of select="field_name"/>
                  </xsl:when>
                  <xsl:otherwise>
                      <xsl:value-of select="field_name"/>
                  </xsl:otherwise>
            ...
            <xsl:choose>
              ...
              <xsl:when test="(not(field_array_indexes) and not(contains(./rel_range, ',') or contains(./rel_range, ':'))) or field_array_indexes/@element_size = 1">
                <xsl:value-of select="$HEADER_TEXT"/><xsl:text>, bit [</xsl:text>
              </xsl:when>
              <xsl:otherwise>
                <xsl:value-of select="$HEADER_TEXT"/><xsl:text>, bits [</xsl:text>
              </xsl:otherwise>
            </xsl:choose>
          </xsl:when>
          <xsl:otherwise>
      ...
      <xsl:variable name="condition_header">
        <xsl:choose>
          <xsl:when test="./fields_condition">
      ...
      <xsl:choose>
        <xsl:when test="./parent_description">
          <h4>
            <xsl:attribute name="id">
              <xsl:value-of select="$ID"/>
            </xsl:attribute>
            <xsl:copy-of select="$full_header"/>
          </h4>
          <span>
            <p>
              <xsl:value-of select="./parent_description"/>
            </p>
          </span>
          <h4>
            <xsl:attribute name="id">
            ...
            <xsl:copy-of select="$condition_header"/>
          </h4>
        </xsl:when>
        <xsl:otherwise>
          <h4>
...
<xsl:template match="field_values">
  ...
  <xsl:if test="./field_value_instance">
    ...
    <table class="valuetable">
      <tr>
        <xsl:if test="./field_value_instance/field_value_instance_name">
          <th>Name</th>
        </xsl:if>
        <th>
          <xsl:choose>
      ...
      <xsl:for-each select="field_value_instance">
        <tr>
          <xsl:if test="../field_value_instance/field_value_instance_name">
            <td><xsl:value-of select="field_value_instance_name"/></td>
          </xsl:if>
...
<xsl:template name="field_getter_setter">
  <ul>
    <xsl:if test="field_access_getter">
      <li>On read:
        <p class="pseudocode"><xsl:value-of select="field_access_getter/ps/pstext"/></p>
      </li>
    </xsl:if>
    <xsl:if test="field_access_setter">
      <li>On write:
        <p class="pseudocode"><xsl:value-of select="field_access_setter/ps/pstext"/></p>
      </li>
    </xsl:if>
  </ul>
</xsl:template>
...
<xsl:template match="field_access">
  <xsl:choose>
    <!-- Single unconditioned getter setter case -->
    <xsl:when test="not(count(field_access_state) > 1) and not(field_access_state/field_access_type) and not(field_access_state/field_access_level)">
      <p>Accessing this field has the following behavior:</p>
      <xsl:for-each select="field_access_state">
        <xsl:call-template name="field_getter_setter"/>
      </xsl:for-each>
    </xsl:when>
    <!-- Multi access case -->
    <xsl:when test="count(field_access_state) > 1 or count(field_access_state/field_access_level/field_access_sublevel) > 1 or not(count(field_access_state/field_access_type) > 0)">
      <p>Accessing this field has the following behavior:</p>
      <ul>
        <xsl:for-each select="field_access_state">
          <li>
            <xsl:choose>
              <xsl:when test="field_access_level">
              <xsl:choose>
                <xsl:when test="not(field_access_type)">
                  <xsl:apply-templates select="field_access_level"/>
                  <xsl:call-template name="field_getter_setter"/>
                </xsl:when>
                <xsl:when test="not(count(field_access_type/field_access_state) > 0)">
    ...
    <xsl:choose>
        <xsl:when test="count(../field_access_type/field_access_state) > 0">
          <xsl:choose>
            ...
            <!-- Case with nested field_access_type but no sublevel's -->
            <xsl:otherwise>
              <xsl:value-of select="."/>:
            </xsl:otherwise>
          </xsl:choose>
        ...
        <xsl:when test="count(field_access_sublevel) > 0">
          ...
          <xsl:text> to this field </xsl:text><xsl:apply-templates select="$access_value"/><xsl:text> if </xsl:text>
          <xsl:choose>
          <xsl:when test="@operand='AND'">
          ...
          <xsl:text> the following are true:</xsl:text>
          <ul>
            <xsl:for-each select="field_access_sublevel">
        ...
        <xsl:when test="not(count(../field_access_type) > 0)">
          <xsl:value-of select="."/>:
        </xsl:when>
        <xsl:otherwise>
